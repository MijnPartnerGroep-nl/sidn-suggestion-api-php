...
class Authenticate extends BaseResource {
    ...
    /**
     * This API uses the Client Authorization flow for server-side authorization. To start the authorization flow, you need to request a clientId and clientSecret. You can request these from SIDN Support. 
     * 
    ...
    public function Authenticate($client_id, $client_secret, $scope = "suggestions", $grant_type = "client_credentials") {
        ...
        $result = $this->client->ep_authenticate->post(array("grant_type" => $grant_type, "scope" => $scope), array("Authorization" => "Basic ".base64_encode($client_id.":".$client_secret)));
